...
def process_data(dataset_path: str, csv_file: str, vocab_path: str, stopwords_path: str, words_path: str, labels_path: str, embedding_size: int) -> None:
    ...
    # get only english transcripts from transcripts file
    en_sentences = []
    ...
    labels = ['O' for _ in range(len(dataset_words))] # create labels array

    # clear dataset and delete word conjuctions (fabio's -> fabio is)
    processed_words = [w for w in word_tokenize(en_dataset) if not w.startswith('[')]
    
    # read usefull data to process dataset (soccer data, ner tags)
    ...
    # words from soccer data that appear in the dataset and are not stopwords
    possible_labels = set()
    soccer_values = [item for values in soccer_data.values() for item in values]
    ...
    # label each dataset word according to the soccer data
    ...
    # split dataset into sentences of x words
